...
Public Class frmLotAdditionIssue
    ...
    Private Sub cmbLotNo_SelectedIndexChanged(sender As Object, e As Data.PositionChangedEventArgs) Handles cmbLotNo.SelectedIndexChanged
        ...
        If cmbLotNo.Items.Count > 0 Then
            fillHeaderFromListView(cmbLotNo.Text.Trim())
        End If
    ...
    Private Sub UpdateData()
        ...
        Dim iOperationTypeId As Integer = 22 ''Operation TypeId for Lot Addition Issue
        ...
        alParaval.Add(LotADt.Value.Date)
        alParaval.Add(iOperationId)
        alParaval.Add(cmbLotNo.Text.Trim)
        ...
        For Each row As GridViewRowInfo In dgvLotAddition.Rows
            If row.Cells(0).Value <> Nothing Then
                ...
                Else
                    ...
                    FineWt = FineWt & "|" & Val(row.Cells(7).Value)
                    Remarks = Remarks & "|" & (row.Cells(8).Value)
                    LineNo = LineNo & "|" & Val(row.Cells(9).Value)
                End If
            End If
            IRowCount += 1
        Next
    ...
    End Function
    Private Sub fillHeaderFromListView(ByVal sLotNo As Integer)

        Dim parameters = New List(Of SqlParameter)()
        parameters.Clear()

        parameters.Add(dbManager.CreateParameter("@ActionType", "FetchHeaderRecord", DbType.String))
        parameters.Add(dbManager.CreateParameter("@LotNo", CStr(sLotNo), DbType.String))
        ...
        If dr.HasRows = True Then
            dr.Read()
            txtLotIssueId.Tag = dr.Item("ItemId").ToString
...
ErrHandler:
    ...
    Private Sub lstLotAddition_DoubleClick(sender As Object, e As EventArgs) Handles lstLotAddition.DoubleClick
        If lstLotAddition.Items.Count > 0 Then
            ...
            fillDetailsFromListView(LotIssueId)
    ...
    Private Sub fillDetailsFromListView(ByVal sLotNo As String)
        ...
        For Each Row As DataRow In dttable.Rows
            dgvLotAddition.Rows.Add(1, Val(Row("ItemId")), CStr(Row("ItemName")), Format(Val(Row("IssueWt")), "0.00"), Format(Val(Row("IssuePr")), "0.00"), Format(Val(Row("FineWt")), "0.00"), Row("Remarks"), Row("IsChecked"))
        Next
    ...
    Private Sub txtIssueWt_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles txtIssueWt.Validating
        ...
        End If
    End Sub
    Private Sub lstLotAddition_ColumnWidthChanging(sender As Object, e As ColumnWidthChangingEventArgs) Handles lstLotAddition.ColumnWidthChanging
        Dim DisableColumns As Integer() = {0, 1, 2, 3, 4}

        For Each DCol As Integer In DisableColumns
            If e.ColumnIndex = DCol Then
                e.Cancel = True
                e.NewWidth = lstLotAddition.Columns(DCol).Width
            End If
        Next DCol
    End Sub
    ...
    Private Sub dgvLotAddition_CellDoubleClick(sender As Object, e As GridViewCellEventArgs) Handles dgvLotAddition.CellDoubleClick
        Try
            ...
            If e.RowIndex >= 0 And dgvLotAddition.Rows(e.RowIndex).Cells(0).Value <> Nothing And dgvLotAddition.Rows(e.RowIndex).Cells(7) Then
                GridDoubleClick = True

                txtSrNo.Text = dgvLotAddition.Rows(e.RowIndex).Cells(0).Value.ToString()
                cmbItemType.SelectedIndex = dgvLotAddition.Rows(e.RowIndex).Cells(1).Value.ToString()
                cmbItemType.Text = dgvLotAddition.Rows(e.RowIndex).Cells(2).Value.ToString()
                txtIssueWt.Text = dgvLotAddition.Rows(e.RowIndex).Cells(3).Value.ToString()
                txtIssuePr.Text = dgvLotAddition.Rows(e.RowIndex).Cells(4).Value.ToString()
                txtFineWt.Text = dgvLotAddition.Rows(e.RowIndex).Cells(5).Value.ToString()
                txtRemark.Text = dgvLotAddition.Rows(e.RowIndex).Cells(6).Value.ToString()
                TempRow = e.RowIndex
                cmbItemType.Focus()
            End If
        Catch ex As Exception
