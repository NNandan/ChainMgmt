...
Public Class frmVaccumBag
    ...
    Private Sub bindVacuumBag()
        ...
        parameters.Add(dbManager.CreateParameter("@ActionType", "GetVaccumData", DbType.String))
        parameters.Add(dbManager.CreateParameter("@BId", Val(cmbCBagtype.SelectedValue), DbType.Int16))
    ...
    End Sub
    'Private Sub GetBagPrefix()

    '    Dim parameters = New List(Of SqlParameter)()
    '    parameters.Clear()

    '    With parameters
    '        .Add(dbManager.CreateParameter("@ActionType", "SearchBagPrefixByBagId", DbType.String))
    '        .Add(dbManager.CreateParameter("@BId", Val(cmbCBagtype.SelectedValue), DbType.Int16))
    '    End With

    '    Dim dr As SqlDataReader = dbManager.GetDataReader("SP_BagMaster_Select", CommandType.StoredProcedure, Objcn, parameters.ToArray())
    ...
    '    Try
    '        While dr.Read
    '            If Not IsDBNull(dr.Item("ItemCode")) Then
    '                txtMaxNo.Text = dr.Item("ItemCode").ToString()
    '            End If
    '        End While
    ...
    '    End Try
    'End Sub
    'Private Function GetMaxVacuumBagNo() As String

    '    Dim ReceiveNo As String = String.Empty

    '    Dim parameters = New List(Of SqlParameter)()
    '    parameters.Clear()

    '    With parameters
    '        .Add(dbManager.CreateParameter("@ActionType", "GetMaxBhukaBagNo", DbType.String))
    '        .Add(dbManager.CreateParameter("@BId", CInt(cmbCBagtype.SelectedValue), DbType.Int16))
    '    End With

    '    ReceiveNo = Convert.ToString(dbManager.GetScalarValue("SP_Transaction_Select", CommandType.StoredProcedure, parameters.ToArray()))

    '    'If String.IsNullOrEmpty(ReceiveNo) Then
    '    '    GetMaxVacuumBagNo = txtMaxNo.Text.Trim() & "0001"
    '    'Else
    '    '    GetMaxVacuumBagNo = txtMaxNo.Text.Trim() & Format(Val(ReceiveNo.Substring(3, 4)) + 1, "0000")
    '    'End If
