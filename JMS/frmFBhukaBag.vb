...
Public Class frmFBhukaBag
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Try
            If lblMaxNo.Visible = False And txtMaxNo.Visible = False And btnSave.Text = "&Save" Then
                ...
                Else
                    If Not Validate_EField() Then Exit Sub
                    Me.UpdateData()
                    Me.btnCancel_Click(sender, e)
                End If
    ...
    Private Function SaveData() As DataTable
        ...
        Try
            ...
            Dt = dbManager.GetDataTable("SP_fScrapBags_Save", CommandType.StoredProcedure, Hparameters.ToArray())
            ...
            Try
                ...
                strSQL = "Select Max(BagSrNo) from tblfScrapBags where BagID=(select Distinct ItemCode from tblItemMaster where ItemId=" + ScrapItemId + ")"
                Try
    ...
    Private Sub UpdateData()
        ...
        Try
            ...
            With Hparameters
                ...
                'iValue += 1
                .Add(dbManager.CreateParameter("@BagType", "B", DbType.String))
                .Add(dbManager.CreateParameter("@DItemName", (cmbItemScrapBag.Text), DbType.String))
                .Add(dbManager.CreateParameter("@NItemId", alParaval.Item(iValue), DbType.String))
                iValue += 1
                .Add(dbManager.CreateParameter("@NItemName", alParaval.Item(iValue), DbType.String))
                iValue += 1
                .Add(dbManager.CreateParameter("@NGrossWt", alParaval.Item(iValue), DbType.String))
                iValue += 1
                .Add(dbManager.CreateParameter("@NGrossPr", alParaval.Item(iValue), DbType.String))
                ...
                .Add(dbManager.CreateParameter("@GridCount", IRowCount, DbType.Int16))
            End With
            Dt = dbManager.GetDataTable("SP_fScrapBags_Update", CommandType.StoredProcedure, Hparameters.ToArray())
        ...
        '    Dim sresult As String = String.Empty
        '    strSQL = Nothing
        '    Dim ScrapItemName As String
        '    ScrapItemName = cmbItemScrapBag.Text.ToString()
        '    strSQL = "Select Max(BagSrNo) from tblfScrapBags where BagID=(select Distinct ItemCode from tblItemMaster where ItemName='" + ScrapItemName + "')"
        '    Try
        '        sresult = Convert.ToString(dbManager.GetScalarValue(strSQL, CommandType.Text))
        '        Int32.TryParse(sresult, icurValue)
        '        Dim ScrapBagNo = sresult
        '        MessageBox.Show("ScrapBag " + ScrapBagNo + " Updated !!!", "Updated", MessageBoxButtons.OK, MessageBoxIcon.Information)
        '    Catch ex As Exception
        '        MessageBox.Show("Error:- " & ex.Message)
        '    End Try
        'Catch
    ...
    Private Sub bindVacuumBagWt()
        ...
        Try
            ...
            dtData = dbManager.GetDataTable("SP_fScrapBags_Select", CommandType.StoredProcedure, parameters.ToArray())
            ...
            If dtData.Rows.Count > 0 Then
                ...
                Try
                    Dim sresult As String = String.Empty
                    strSQL = Nothing
                    strSQL = "select tblItemMaster.ItemName from tblfScrapBags inner join tblItemMaster ON tblItemMaster.ItemCode=tblfScrapBags.BagId where tblfScrapBags.BagSrNo='" + Convert.ToString(cmbRBagNo.Text) + "'"
                    sresult = Convert.ToString(dbManager.GetScalarValue(strSQL, CommandType.Text))
        ...
        Try
            ...
            If dtData.Rows.Count > 0 Then
                ...
                Try
                    Dim sresult As String = String.Empty
                    strSQL = Nothing
                    strSQL = "select tblItemMaster.ItemName from tblfUsedBags inner join tblItemMaster ON tblItemMaster.ItemCode=tblfUsedBags.BagId where tblfUsedBags.BagSrNo='" + Convert.ToString(cmbUBagNo.Text) + "'"
                    sresult = Convert.ToString(dbManager.GetScalarValue(strSQL, CommandType.Text))
                    Dim VaccumeBagName = sresult
    ...
    Private Function Validate_CFields() As Boolean
        ...
        End Try
    End Function
    Private Function Validate_EField() As Boolean
        Try
            If Not dgvBhuka.RowCount > 0 Then
                MessageBox.Show("Cannot Save Without Detail Information!!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.[Error])
                Return False
            End If
            If cmbItemScrapBag.Text = "---Select---" Then
                MessageBox.Show("Please Select Bag!!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.[Error])
                Return False
            End If
            Return True
    ...
    Private Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        ...
        If btnEdit.Text = "Edit" Then
            ...
            Else
                ...
                cmbItemScrapBag.SelectedValue = Me.cmbRBagNo.EditorControl.CurrentRow.Cells(0).Value
                cmbItemScrapBag.Text = Me.cmbRBagNo.EditorControl.CurrentRow.Cells(2).Value
                btnSave.Text = "Update"
        ...
        Try
            ...
            If btnEdit.Text = "Update" Then
                With parameters
                    ...
                    .Add(dbManager.CreateParameter("@UCreatedBy", UserName.Trim(), DbType.String))
                End With
                dbManager.Update("SP_fUsedBags_Update", CommandType.StoredProcedure, parameters.ToArray())
